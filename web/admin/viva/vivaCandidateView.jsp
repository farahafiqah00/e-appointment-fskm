<%-- Admin: read-only detail view of a single viva candidate including co-supervisors and status history. --%>
...
<%
    ...
    if (!idStr.isEmpty()) {
        try {
            c = new CandidateDAO().findById(Integer.parseInt(idStr));
    ...
    String progLevel = c.getProgramLevel(); // "PhD", "Master", or null
    String flashCompleted = request.getParameter("completed");
...
<html lang="en">
  <head>
    ...
    <title>View Viva Candidate - E-Appointment FSKM</title>
        ...
        <div class="ea-main-content-centered d-flex flex-column w-100" style="align-items:stretch;max-width:900px;margin:0 auto;">
          ...
          <div class="d-flex align-items-start justify-content-between mb-3 flex-wrap gap-3">
            <div>
              ...
              <div style="font-size:1rem;color:#6b7280;">Viewing viva candidate record</div>
            </div>
            <div class="d-flex gap-2 flex-wrap">
              <a href="<%= request.getContextPath() %>/CandidateListServlet" class="btn-ea-back">
                <i class="bi bi-arrow-left"></i> Back to list
              </a>
              <% if ("prepared".equals(displayStatus)) { %>
              ...
              <form method="POST" action="<%= request.getContextPath() %>/MarkCandidateCompletedServlet"
                ...
                <button type="submit" style="background:#fff;color:#16a34a;border:1.5px solid #16a34a;border-radius:10px;padding:8px 18px;font-weight:600;cursor:pointer;font-size:0.95rem;display:inline-flex;align-items:center;gap:0.4rem;line-height:1.5;">
          ...
          <div class="alert d-flex align-items-center gap-2 mb-3"
            ...
            Could not update the candidate status. Please try again.
          </div>
          <% } %>
            ...
            <div class="row g-4">
              ...
              <div class="col-md-6">
                ...
                <div>
                  ...
                  <% } else { %>
                  <span style="background:#fef3c7;color:#d97706;padding:4px 14px;border-radius:20px;font-size:0.9rem;font-weight:600;">Prepared</span>
                  <% } %>
                </div>
              </div>

              <!-- Matric + Programme row -->
              <div class="col-md-6">
              ...
              <div class="col-md-6">
                ...
                <div class="d-flex flex-wrap align-items-center gap-2">
                  ...
                  <% } %>
                  <% if ("PhD".equalsIgnoreCase(progLevel)) { %>
                  <span style="background:#ede9fe;color:#6d28d9;padding:3px 10px;border-radius:20px;font-size:0.78rem;font-weight:600;">PhD</span>
                  <% } else if ("Master".equalsIgnoreCase(progLevel)) { %>
                  <span style="background:#fef3c7;color:#92400e;padding:3px 10px;border-radius:20px;font-size:0.78rem;font-weight:600;">Master</span>
                  <% } %>
                </div>
              </div>
              ...
              <!-- Nationality -->
              <div class="col-md-6">
                <div style="font-size:0.82rem;font-weight:600;color:#9ca3af;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:4px;">Nationality</div>
                <div style="color:#374151;">
                  <%= (c.getNationality() != null && !c.getNationality().isEmpty()) ? c.getNationality() : "—" %>
                </div>
              </div>
